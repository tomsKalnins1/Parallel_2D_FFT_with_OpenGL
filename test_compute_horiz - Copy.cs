...
void permutate(){
    ...
    while(num < gl_WorkGroupSize.x ){
        ...
        ivec2 texCoor_l = ivec2(gl_LocalInvocationID.xy);
    for(int i = 0; i < gl_WorkGroupSize.x/2; i++ ){
    
        vec4  temp = texelFetch(screen, texCoor, 0);
        vec4 reversed = texelFetch(screen, ivec2(rev(texCoor.x, bits), texCoor.y), 0);
      //  reversed = vec4(1.0, 0.2, 0.4, 1.0);
        imageStore(screen_out, texCoor, reversed);
        imageStore(screen_out, ivec2(rev(texCoor.x, bits), texCoor.y), temp);
...
void permutate1(){
    ...
    while(num <= gl_WorkGroupSize.x - 1 ){
     ...
     //   cache[rev(texCoor.x, bits)] = temp;

        real_imag[rev(texCoor.x, bits)] = vec2(temp.x, 0.0);
...
void fft(){
    ...
    uint k = 2;
    uint num_lvls = uint(log2(gl_WorkGroupSize.x));
    ...
    for(uint lvl = 0; lvl < num_lvls; lvl++){
         ...
         if( ( gl_LocalInvocationIndex ^ k )% k < k/2 ){
          ...
          //  v = vec4(1.0, 0.0, 0.0, 1.0);
           ...
           vec2 freq_1 = even_1 - mult_1( odd_1, twiddle );
...
void main()
{
  ...
  //  imageStore(screen_out, texC_g, cache[gl_LocalInvocationIndex]);
    ...
    imageStore(fft_data, texC_g, v);
    //vec4 pix = texelFetch(screen, texC_g, 0);
    //pix = vec4(pix.x * 0.5, pix.y * 0.1, pix.z * 0.0, 1.0);
     //imageStore(fft_data, texC_g, pix);

    // permutate();
    // ivec2 texC_g = ivec2(gl_GlobalInvocationID.xy);

    //  vec4 color = texelFetch(screen, texC_g, 0);
    // float loc_invoc = float(gl_LocalInvocationIndex) / 256.0;
    //  vec4 v = vec4(color.x * loc_invoc, color.y * loc_invoc, color.z * loc_invoc , 1.0);

    //  imageStore(screen_out, texC_g, v);
}
